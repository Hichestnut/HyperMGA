...
for n_perturbations in range(2, 11):
    ...
    running_time = 0
    for index, target_node in enumerate(points):
        rddelH, rt = randomDelete(H, target_node, n_perturbations)
        running_time = running_time + rt
        rddelG = hgut._generate_G_from_H(rddelH, variable_weight=False)
        rddelG = torch.Tensor(rddelG).to(device)
        for i in range(0,10):
          randomresult = target_model(fts, rddelG)
          if (torch.argmax(result[target_node]) != torch.argmax(randomresult[target_node])):
              suc_attack = suc_attack + 1
    rddel_suc.append(suc_attack / (num * 10.0))
    rddel_time.append(running_time)

print('The ASR of RandomDelete in' + datasets + ':')
print(rddel_suc)
print(rddel_time)
...
for n_perturbations in range(2, 11):
    ...
    running_time = 0
    for index, target_node in enumerate(points):
        dcatkH, rt = diceAttack(H, target_node, n_perturbations)
        running_time = running_time + rt
        dcatkG = hgut._generate_G_from_H(dcatkH, variable_weight=False)
        dcatkG = torch.Tensor(dcatkG).to(device)
        for i in range(0,10):
          randomresult = target_model(fts, dcatkG)
          if (torch.argmax(result[target_node]) != torch.argmax(randomresult[target_node])):
              suc_attack = suc_attack + 1
    dcatk_suc.append(suc_attack / (num * 10.0))
    dcatk_time.append(running_time)

print('The ASR of DiceAttack in' + datasets + ':')
print(dcatk_suc)
print(dcatk_time)
